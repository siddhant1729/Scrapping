...
Automatic topic generation is performed via a hybrid NLP approach in `utils/`:
...
2. **Frequency Fallback**: To ensure resilience (especially during NumPy/SciPy version conflicts), we implement a fallback frequency analyzer in `utils/tagging.py`. This ensures that the scraper remains functional even in restricted environments.
...
| Medical Disclaimer Presence | **0.10** | Presence of disclaimers for ethical/regulatory safety. |
...
- **Author Credibility**: Verified against `scoring/trusted_orgs.json`. Supports **multi-author averaging** where individual scores are calculated and the mean is returned.
...
- **Medical Disclaimer Presence**: A boolean keyword scan for phrases like "Not medical advice". PubMed is automatically granted 1.0 due to its peer-reviewed nature.
