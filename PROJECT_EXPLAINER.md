...
2. **Parsers (`parsers/`)**: Isolated modules that contain source-specific logic. This keeps the code "decoupled"—you can add a new source (like Twitter or LinkedIn) simply by adding a new parser file.
3. **Common Utilities (`utils.py`)**: Shared logic (cleaning, chunking, tagging) that all parsers call to ensure data consistency.
...
- [x] **Cross-Ecosystem**: Does it handle Blogs, Video, and Science? **Yes, via 3 distinct parsers.**
